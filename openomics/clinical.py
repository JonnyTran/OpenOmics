...
class ClinicalData:
    ...
    def __init__(
        self, patients_file, patient_id_col="bcr_patient_barcode", columns=None
    ):
        ...
        self.patient.replace(
            {PATHOLOGIC_STAGE: ClinicalData.pathologic_stage_map}, inplace=True
        )
    ...
    def build_clinical_samples(self, all_samples, index="bcr_patient_barcode"):
        ...
        self.samples = self.samples.join(
            self.patient, on=index, how="left", rsuffix="_"
        )
        ...
        self.samples.loc[
            self.samples.index.str.contains("-11"), TUMOR_NORMAL
        ] = NORMAL  # Change stage label of normal samples to "Normal"
        self.samples.loc[
            self.samples.index.str.contains("-01"), TUMOR_NORMAL
        ] = TUMOR  # Change stage label of normal samples to "Normal"
