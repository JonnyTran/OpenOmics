...
class Interactions(Dataset):
    ...
    def filter_values(self, df: pd.DataFrame, filters: dict, case=False):
        ...
        assert (
            df.shape[0] > 0
        ), f"ERROR: Dataframe is empty because of filter: {filters}"
        return df
...
class GeneMania(Interactions):
    def __init__(
        ...
        if file_resources is None:
            file_resources = {}
            file_resources["COMBINED.DEFAULT_NETWORKS.BP_COMBINING.txt"] = os.path.join(
    ...
    def load_network(
        ...
        id_mapping = pd.Series(
            identifier["Name"].values, index=identifier["Preferred_Name"]
        ).to_dict()
...
class STRING(Interactions, SequenceDataset):
    ...
    def __init__(
        ...
        if file_resources is None:
            ...
            file_resources["protein.links.txt"] = os.path.join(
                path,
                "protein.links.v11.0/{}.protein.links.v11.0.txt.gz".format(species_id),
            )
            file_resources["protein.info.txt"] = os.path.join(
                path,
                "protein.info.v11.0/{}.protein.info.v11.0.txt.gz".format(species_id),
            )
    ...
    def load_network(
        ...
        logging.info(
            "{}: {}".format(self.name(), protein_interactions.columns.tolist())
        )
        protein_info = pd.read_table(file_resources["protein.info.txt"])

        self.protein_id2name = protein_info.set_index("protein_external_id")[
            "preferred_name"
        ].to_dict()
        network = nx.from_pandas_edgelist(
...
class LncBase(Interactions, Dataset):
    ...
    def get_rename_dict(self, from_index="geneId", to_index="geneName"):
        ...
        gene_id_to_gene_name_dict = pd.Series(
            lncbase_df["geneName"].values, index=lncbase_df["geneId"]
        ).to_dict()
...
class lncRInter(Interactions):
    ...
    def load_network(
        ...
        lncRInter_df["Interacting partner"] = lncRInter_df[
            "Interacting partner"
        ].str.replace("mirlet", "hsa-let-")
        lncRInter_df["Interacting partner"] = lncRInter_df[
            "Interacting partner"
        ].str.replace("mir", "hsa-mir-")
...
class LncRNA2Target(Interactions):
    ...
    def load_network(
        ...
        if self.version == "high_throughput":
            return self.load_network_high_throughput(
                file_resources, source_col_name, target_col_name, edge_attr, directed
            )
        elif self.version == "low_throughput":
            return self.load_network_low_throughput(
                file_resources, source_col_name, target_col_name, edge_attr, directed
            )
        else:
            raise Exception(
...
class NPInter(Interactions):
    ...
    def load_network(
        ...
        df = pd.read_table(
            file_resources["interaction_NPInterv4.expr.txt"], header=0, na_values=["-"]
        )
        logging.info(f"{self.name()}, {df.columns.tolist()}")
...
class StarBase(Interactions):
    def __init__(
        ...
        if file_resources is None:
            file_resources = {}
            file_resources["starbase_3.0_lncrna_rna_interactions.csv"] = os.path.join(
    ...
    def load_network(
        ...
        df = pd.read_csv(
            self.file_resources["starbase_3.0_lncrna_rna_interactions.csv"], header=0
        )

        df.loc[df["pairGeneType"] == "miRNA", "pairGeneName"] = df[
            df["pairGeneType"] == "miRNA"
...
class TargetScan(Interactions, Dataset):
    def __init__(
        ...
        if file_resources is None:
            ...
            ] = os.path.join(path, "Predicted_Targets_Info.default_predictions.txt")
    ...
    def process_interactions_table(self, file_resources, family_to_miR_df, species):
        ...
        mir_interactions_df = family_interactions_df.join(
            family_to_miR_df, how="outer", on="miR Family"
        ).reset_index()
