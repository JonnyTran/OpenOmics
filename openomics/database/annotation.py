...
class TANRIC(Dataset):
    def __init__(
        ...
        Args:
            ...
            verbose:
        """
        super(TANRIC, self).__init__(
            path, file_resources, col_rename, npartitions, verbose
        )
...
class ProteinAtlas(Dataset):
    ...
    def __init__(
        ...
        super(ProteinAtlas, self).__init__(
            path, file_resources, col_rename, npartitions, verbose
        )
...
class RNAcentral(Dataset):
    ...
    def __init__(
        ...
        if file_resources is None:
            ...
            file_resources[
                "database_mappings/gencode.tsv"
            ] = "id_mapping/database_mappings/gencode.tsv"
            file_resources[
                "database_mappings/mirbase.tsv"
            ] = "id_mapping/database_mappings/mirbase.tsv"

        super(RNAcentral, self).__init__(
            path,
            file_resources,
    ...
    def load_dataframe(self, file_resources, npartitions=None):
        ...
        lnc_go_terms = (
            go_terms[go_terms["RNAcentral id"].isin(gene_ids["RNAcentral id"])]
        ...
        gene_ids = gene_ids[
            gene_ids["GO terms"].notnull() | gene_ids["Rfams"].notnull()
        ]
...
class GTEx(Dataset):
    ...
    def __init__(
        ...
        if file_resources is None:
            ...
            file_resources[
                "GTEx_Analysis_2017-06-05_v8_RSEMv1.3.0_transcript_tpm.gct"
    ...
    def load_dataframe(
        self, file_resources, npartitions=None
        ...
        gene_exp_medians = gene_exp_medians.rename(
            columns=self.COLUMNS_RENAME_DICT
        )  # Must be done here
...
class NONCODE(Dataset):
    def __init__(
        ...
        super(NONCODE, self).__init__(
            path, file_resources, col_rename, verbose=verbose, npartitions=npartitions
        )
    ...
    def load_dataframe(self, file_resources, npartitions=None):
        ...
        self.noncode_func_df["NONCODE Transcript ID"] = self.noncode_func_df.index.map(
            pd.Series(
                ...
                index=transcript2gene_df["NONCODE Gene ID"],
            ).to_dict()
        )
        ...
        self.noncode_func_df["Gene Name"] = self.noncode_func_df[
            ...
            pd.Series(
                source_gene_names_df["Gene ID"].values,
...
class BioMartManager:
    ...
    def query_biomart(
        ...
        print(
            "Querying {} from {} with attributes {}...".format(
...
class EnsemblGenes(BioMartManager, Dataset):
    ...
    def load_data(self, dataset, attributes, host, filename=None, npartitions=None):
        ...
        return self.retrieve_dataset(
            host, dataset, attributes, filename, npartitions=npartitions
        )
    ...
    def get_rename_dict(self, from_index="gene_id", to_index="gene_name"):
        ...
        """
        geneid_to_genename = (
            self.data[self.data[to_index].notnull()]
            .groupby(from_index)[to_index]
            .apply(concat_uniques)
            .to_dict()
        )
    ...
    def get_functional_annotations(self, index):
        ...
        """
        geneid_to_go = (
            self.data[self.data["go_id"].notnull()]
            .groupby(index)["go_id"]
            .apply(lambda x: "|".join(x.unique()))
            .to_dict()
        )
