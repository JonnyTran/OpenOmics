...
class TANRIC(Dataset):
    def __init__(self,
        ...
        Args:
            ...
            verbose:
        """
        super(TANRIC, self).__init__(path, file_resources, col_rename,
                                     npartitions, verbose)
...
class ProteinAtlas(Dataset):
    ...
    def __init__(self,
        ...
        super(ProteinAtlas, self).__init__(path, file_resources, col_rename,
                                           npartitions, verbose)
    ...
    def get_expressions(self, index="gene_name", type="Tissue RNA"):
        ...
        expressions = self.data.filter(regex=columns).groupby(index).median()
        return expressions
...
class RNAcentral(Dataset):
    ...
    def __init__(
        ...
        super(RNAcentral, self).__init__(path,
                                         file_resources,
...
class GTEx(Dataset):
    COLUMNS_RENAME_DICT = {"Name": "gene_id", "Description": "gene_name"}
    ...
    def load_dataframe(self,
                       file_resources,
                       npartitions=None):  # type: (dict) -> pd.DataFrame
        """
        Args:
            file_resources:
            npartitions:
        """
        gene_exp_medians = pd.read_csv(self.file_resources[
        ...
        gene_exp_medians["Name"] = gene_exp_medians["Name"].str.replace(
            "[.].*", "")
        gene_exp_medians = gene_exp_medians.rename(
            columns=self.COLUMNS_RENAME_DICT)  # Must be done here
...
class NONCODE(Dataset):
    def __init__(self,
        ...
        if file_resources is None:
            file_resources = {}
            file_resources["NONCODEv5_source"] = os.path.join(
                path, "NONCODEv5_source")
            file_resources["NONCODEv5_Transcript2Gene"] = os.path.join(
                path, "NONCODEv5_Transcript2Gene")
            file_resources["NONCODEv5_human.func"] = os.path.join(
                path, "NONCODEv5_human.func")

        super(NONCODE, self).__init__(path,
                                      file_resources,
                                      col_rename,
                                      verbose=verbose,
                                      npartitions=npartitions)
...
class BioMartManager:
    ...
    def retrieve_dataset(self,
        ...
        if os.path.exists(filename):
            ...
            df = self.query_biomart(host=host,
                                    dataset=dataset,
    ...
    def cache_dataset(self, dataset, dataframe, save_filename):
        ...
        if save_filename is None:
            save_filename = os.path.join(DEFAULT_CACHE_PATH,
                                         "{}.tsv".format(dataset))
    ...
    def query_biomart(self,
        ...
        if npartitions:
            df = dd.read_csv(StringIO(results),
                             header=None,
                             names=attributes,
                             sep="\t")
        else:
...
class EnsemblGenes(BioMartManager, Dataset):
    ...
    def __init__(self,
        ...
        self.host = host
        self.data = self.load_data(dataset=biomart,
                                   attributes=attributes,
                                   host=self.host,
    ...
    def load_data(self,
        ...
        Args:
            ...
            npartitions:
        """
        return self.retrieve_dataset(host,
                                     dataset,
                                     attributes,
                                     filename,
                                     npartitions=npartitions)
...
class EnsemblGeneSequences(EnsemblGenes):
    def __init__(self,
        ...
        if attributes is None:
            attributes = [
                'ensembl_gene_id', 'gene_exon_intron', 'gene_flank',
                'coding_gene_flank', 'gene_exon', 'coding'
            ]
        self.filename = "{}.{}".format(biomart, self.__class__.__name__)
        self.host = host
        self.df = self.load_data(dataset=biomart,
                                 attributes=attributes,
                                 host=self.host,
                                 filename=self.filename,
                                 npartitions=npartitions)
...
class EnsemblTranscriptSequences(EnsemblGenes):
    def __init__(self,
        ...
        if attributes is None:
            attributes = [
                'ensembl_transcript_id', 'transcript_exon_intron',
                'transcript_flank', 'coding_transcript_flank', '5utr', '3utr'
            ]
        self.filename = "{}.{}".format(biomart, self.__class__.__name__)
        self.host = host
        self.df = self.load_data(dataset=biomart,
...
class EnsemblSNP(EnsemblGenes):
    def __init__(self,
        ...
        if attributes is None:
            attributes = [
                'synonym_name', 'variation_names', 'minor_allele',
                'associated_variant_risk_allele', 'ensembl_gene_stable_id',
                'ensembl_transcript_stable_id', 'phenotype_name', 'chr_name',
...
class EnsemblSomaticVariation(EnsemblGenes):
    def __init__(self,
        ...
        if attributes is None:
            attributes = [
                'somatic_variation_name', 'somatic_source_name',
                'somatic_allele', 'somatic_minor_allele',
                'somatic_clinical_significance', 'somatic_validated',
                'somatic_transcript_location', 'somatic_mapweight',
