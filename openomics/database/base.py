...
class Dataset(object):
    # Needs initialization since subclasses may use this field to rename columns in dataframes.
    COLUMNS_RENAME_DICT = None
            ...
            for filename, filepath in copy.copy(file_resources).items():
                ...
                # Dask will automatically handle uncompression at dd.read_table(compression=filepath_ext)
                    ...
                    for subfile in zf.infolist():
                        if os.path.splitext(
                                subfile.filename)[-1] == os.path.splitext(
                                    filename
                        )[-1]:  # If the file extension matches
