...
class SequenceDataset(Dataset):
    ...
    def get_aggregator(self, agg=None):
        ...
        if agg == "all":
            ...
            def agg_func(x):
                return list(x) if not isinstance(x, str) else x
        elif agg == "shortest":
...
class GENCODE(SequenceDataset):
    def __init__(
        ...
        super(GENCODE, self).__init__(path=path,
    ...
    def load_dataframe(self, file_resources, npartitions=None):
        ...
        for gtf_file in file_resources:
            if '.gtf' in gtf_file:
                df = read_gtf(file_resources[gtf_file],
                              npartitions=npartitions)
                dfs.append(df)
...
class MirBase(SequenceDataset):
    def __init__(self,
        ...
        super(MirBase, self).__init__(path=path,
                                      file_resources=file_resources,
                                      col_rename=col_rename,
                                      npartitions=npartitions,
                                      replace_U2T=replace_U2T)
    ...
    def load_dataframe(self, file_resources, npartitions=None):
        ...
        rnacentral_mirbase = pd.read_table(
            ...
            header=None,
            names=[
                "RNAcentral id", "database", "mirbase id", "species",
                "RNA type", "NA"
            ])
        ...
        mirbase_aliases = pd.read_table(file_resources["aliases.txt"],
                                        low_memory=True,
                                        header=None,
                                        names=["mirbase id", "gene_name"],
                                        dtype="O").set_index("mirbase id")
