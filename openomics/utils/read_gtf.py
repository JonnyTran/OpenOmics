...
def expand_attribute_strings(
    attribute_strings, quote_char='"', nan_value=None, usecols=None
):
...
def parse_gtf_dask(
    filepath_or_buffer, npartitions=None, compression=None, features=None
):
    """
    Args:
        filepath_or_buffer (str or buffer object):
    ...
    # Uses Dask
    logging.debug(
        f"dask.datafame.read_table, file={filepath_or_buffer}, compression={compression}"
    )
...
def read_gtf(
    ...
    if expand_attribute_column:
        ...
        if npartitions:
            result_df = parse_gtf(
                filepath_or_buffer, features=features, compression=compression
            )
        else:
