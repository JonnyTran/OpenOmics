...
def generate_TCGA_LUAD_SomaticMutation():
    return SomaticMutation(
        data=os.path.join(cohort_folder_path, "LUAD__somaticMutation_geneLevel.txt"),
        transpose=True,
        usecols="GeneSymbol|TCGA",
...
def test_import_expression_table_size(generate_TCGA_LUAD_MessengerRNA):
    ...
    luad_data.add_clinical_data(
        clinical_data=os.path.join(
            cohort_folder_path, "nationwidechildrens.org_clinical_patient_luad.txt"
        )
    )
    ...
    assert (
        luad_data.data[MessengerRNA.name()].shape
...
def test_TCGA_LUAD_multiomics_transcriptomics(generate_TCGA_LUAD):
    ...
    """
    assert all(
        elem in generate_TCGA_LUAD.get_omics_list()
        for elem in [
            MessengerRNA.name(),
            MicroRNA.name(),
            LncRNA.name(),
