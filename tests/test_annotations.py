...
def test_import_rnacentral_db(generate_RNACentral_ftp):
    ...
    """
    assert (
        generate_RNACentral_ftp.data_path
...
def test_import_GTEx(generate_GTEx_expressions):
    ...
    """
    assert (
        generate_GTEx_expressions.data_path
...
def test_import_GeneOntology(generate_GeneOntology):
    ...
    """
    assert (
        generate_GeneOntology.data_path == "http://geneontology.org/gene-associations/"
    )
...
def test_annotate_GeneOntology(generate_TCGA_LUAD, generate_GeneOntology):
    ...
    generate_TCGA_LUAD.LncRNA.annotate_genomics(
        database=generate_GeneOntology, index="gene_id", columns=["go_id"]
    )
